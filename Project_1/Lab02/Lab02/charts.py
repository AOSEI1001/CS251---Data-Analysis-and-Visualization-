...
def sidebarplot(values, labels, title, show_counts=True, figsize=(6, 7)):
    ...
    hbars = ax.barh(np.arange(len(labels)), values, align='center')
...
def sort(values, labels, sort_by='na'):
    ...
    if sort_by=='na':
        ...
        new_l = np.argsort(labels)

        new_values = values[new_l]
        
        new_labels = labels[new_l]
        
        return new_values, new_labels
    
    elif sort_by== 'value':
        new_v = np.argsort(values)

        new_values = values[new_v]
        
        new_labels = labels[new_v]

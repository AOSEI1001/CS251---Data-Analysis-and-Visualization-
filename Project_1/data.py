...
class Data:
    ...
    def read(self, filepath):
        ...
        with open(filepath, "r", encoding="utf-8") as file:
            ...
            self.headers = data[0].replace(" ", "").replace("\n", "").split(",")
            categories = data[1].replace(" ", "").replace("\n", "").split(",")
            
            for x, header in enumerate(self.headers):
                self.header2col[header.strip()] = x 
            
            for x, category in enumerate(categories):
                if category.replace(" ", "") == "categorical":
                    
                    self.cats2levels[self.headers[x]] = []
            ...
            for line in data[2:]:
                ...
                for x, value in enumerate(row):
                    value = value.replace(" ", "")
                    if categories[x].replace(" ", "") == "numeric":
                        parsed.append(float(value) if value else np.nan)
                        
                    elif categories[x].replace(" ", "") == "categorical":
                        if not value:
                            value = "Missing"
                            
                        if value not in self.cats2levels[self.headers[x]]:
                            self.cats2levels[self.headers[x]].append(value)
                                
                        parsed.append(self.cats2levels[self.headers[x]].index(value))
                        
                    # else:
                    #     raise  ValueError(f"Invalid value '{value}' found in column {x} of file. Please check the data!")
                    # am I supposed to change the value using the to string method?
        ...
        for x, value in enumerate(self.headers):
            if categories[x] == "categorical" or categories[x] == "numeric":
                newHeaders.append(value)
        ...
        #parse out necessary data for headers, cats2levels, etc.
    ...
    #change the string method and usr it within the read method 
    ...
    #instead use the data, shape thing since it is equivalent to m
    ...
    def get_header_indices(self, headers):
        ...
        for header in headers: 
            header_indexes.append(self.headers.index(header))
    ...
    def limit_samples(self, start_row, end_row):
        ...
        end_row = min(end_row, self.data.shape[0])
    ...
    def select_data(self, headers, rows=[]):
        ...
        # header_indexes = [self.header2col[header] for header in headers ]
        #or used method you made lol
        header_indexes = []
        ...
        # print(rows)
        selected = self.data[:,header_indexes]
